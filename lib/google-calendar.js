...
function formatMailDate(dateTimeIso) {
...
async function sendGoogleInvitationEmail(payload) {
  const { accessToken, settings } = await getValidGoogleAccessToken();
  const recipient = (payload.to || '').trim();
  if (!recipient) throw new Error('Adresse email manquante pour envoyer l’invitation.');

  const subject = payload.subject || `Invitation ${payload.meetLink ? 'Google Meet' : 'à un échange'} avec L'Atelier`;
  const lines = [
    `Bonjour ${payload.name || ''},`.trim(),
    '',
    payload.intro || 'Voici ton invitation pour notre échange.',
    '',
    `Date : ${formatMailDate(payload.startDateTime)}`,
    payload.meetLink ? `Lien Google Meet : ${payload.meetLink}` : '',
    payload.calendarLink ? `Événement Google Agenda : ${payload.calendarLink}` : '',
    '',
    payload.note || '',
    '',
    `Envoyé par ${payload.senderName || 'L’Atelier CRM'}`,
  ].filter(Boolean);
  const raw = [
    `From: ${settings.connectedEmail || 'me'}`,
    `To: ${recipient}`,
  ...
  const data = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error(data?.error?.message || 'Impossible d’envoyer l’email Gmail');
  return data;
}
